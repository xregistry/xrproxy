...
test('active proxy services include first-wave additions and use unique ports', () => {
  ...
  assert.deepEqual(
    ...
    ['npm', 'pypi', 'maven', 'nuget', 'oci', 'mcp', 'crates', 'terraform', 'rubygems', 'packagist', 'huggingface', 'gomod']
  );
...
test('planned first-wave services have reserved ports and group types', () => {
  ...
  assert.equal(new Set(planned.flatMap(service => service.groupTypes)).size, 1);
});
...
test('image and Docker test matrices are derived from active services', () => {
  ...
  assert.equal(images.length, 14); // 12 active proxies + bridge + bridge-viewer
